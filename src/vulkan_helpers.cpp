...
namespace vuh
{
    ...
    auto loadShader(const vk::Device &device, const char *filename, vk::ShaderModuleCreateFlags flags) -> vk::ShaderModule
    {
    ...
    auto createBuffer(const vk::Device &device, vk::DeviceSize bufSize, vk::BufferUsageFlags usage) -> vk::Buffer
    {
    ...
    auto getComputeQueueFamilyId(const vk::PhysicalDevice &physicalDevice) -> uint32_t
    {
        ...
        auto queue_it = std::find_if(ALL(queueFamilies), [](auto &f) {
            ...
            return 0 < f.queueCount                                               // queue family does have some queues in it
                   && (vk::QueueFlagBits::eCompute & maskedFlags) && !(vk::QueueFlagBits::eGraphics & maskedFlags);
        });
                      ...
                      ) -> uint32_t
    {
        ...
        {
            if ((memoryReqs.memoryTypeBits & (1u << i)) && ((properties & memProperties.memoryTypes[i].propertyFlags) == properties))
            {
                return i;
            }
        }
